    ...
    if (opus_decoder_ != nullptr) {
        ...
        if (ret == ESP_AUDIO_ERR_OK) {
            ...
            if (decoder_sample_rate_ != codec_->output_sample_rate() && output_resampler_ != nullptr) {
                ...
                // 60ms max), producing a ~3KB vector every frame that reliably
                // failed with bad_alloc once heap was pressured. Resample ratio is
                // fixed (16000->24000 = 1.5x), so this needs input_samples*1.5.
...
void AudioService::SetDecodeSampleRate(int sample_rate, int frame_duration) {
    ...
    // A prompt can have a different rate from TTS. Release its converter even
    // when the next stream matches the hardware rate, rather than retaining
    // unused filter state and scratch SRAM throughout the conversation.
    if (output_resampler_ != nullptr) {
        esp_ae_rate_cvt_close(output_resampler_);
