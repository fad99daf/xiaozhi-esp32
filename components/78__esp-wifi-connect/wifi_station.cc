...
void WifiStation::HandleScanResult() {
    ...
    // A hidden AP does not reveal its SSID during a normal scan, so it cannot
    // be matched above. Try the saved credentials directly; the Wi-Fi driver
    // can actively probe for the configured SSID and associate with it.
    if (connect_queue_.empty()) {
        ESP_LOGI(TAG, "No visible known AP found; trying saved credentials for hidden APs");
...
void WifiStation::StartConnect() {
    ...
    if (ap_record.ssid.size() > sizeof(wifi_config.sta.ssid) ||
        ap_record.password.size() > sizeof(wifi_config.sta.password)) {
        ESP_LOGE(TAG, "Skipping invalid WiFi credentials (SSID: %u bytes, password: %u bytes)",
                 static_cast<unsigned>(ap_record.ssid.size()),
                 static_cast<unsigned>(ap_record.password.size()));
        if (!connect_queue_.empty()) {
            StartConnect();
        } else {
            ESP_LOGI(TAG, "No more APs to connect, next scan in %d seconds",
    ...
    // ESP-IDF treats an unset threshold with an 8+ byte password as WPA2-only,
    // which silently filters legacy WPA-PSK APs during connection scanning.
    // WPA-PSK is the lowest supported password-protected mode here; WPA2 and
    // WPA3 APs remain eligible because their authentication modes are stronger.
    wifi_config.sta.threshold.authmode =
        ap_record.password.empty() ? WIFI_AUTH_OPEN : WIFI_AUTH_WPA_PSK;
    if (remember_bssid_ && ap_record.has_bssid) {
