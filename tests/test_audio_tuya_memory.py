...
class TuyaAudioMemoryTests(unittest.TestCase):
    def test_xmini_c3_tuya_uses_native_voice_rate(self):
        # Compile the real board config; both resamplers cost internal SRAM on C3.
        with tempfile.TemporaryDirectory(dir='/tmp') as directory:
            path = Path(directory)
            (path / 'driver').mkdir()
            (path / 'driver/gpio.h').write_text('')
            for tuya, rate in ((1, 16000), (0, 24000)):
                with self.subTest(tuya=tuya):
                    result = subprocess.run(
                        ['c++', '-std=c++17', '-fsyntax-only', '-x', 'c++',
                         f'-DCONFIG_PROTOCOL_TUYA={tuya}', '-I', str(path),
                         '-I', str(ROOT), '-'],
                        input=(
                            '#include "main/boards/xmini-c3/config.h"\n'
                            f'static_assert(AUDIO_INPUT_SAMPLE_RATE == {rate}, '
                            '"Unexpected input rate: Tuya requires a resampler");\n'
                            f'static_assert(AUDIO_OUTPUT_SAMPLE_RATE == {rate}, '
                            '"Unexpected output rate: Tuya requires a resampler");\n'
...
int main() {
    ...
    for (size_t split = 1; split < 80; ++split) {
        ...
        protocol.HandleAudio(input.data(), split, 16000, 40, TAI_STREAM_ONE_SHOT, 1790000001000);
        assert(actual.empty());
        protocol.HandleAudio(input.data()+split, input.size()-split, 0, 0, TAI_STREAM_ONE_SHOT, 1790000001000);
        ...
        protocol.HandleAudio(rest.data(), rest.size(), 0, 0, TAI_STREAM_ONE_SHOT, 1790000001000);
        input.insert(input.end(), rest.begin(), rest.end());
        assert(actual == input);
        assert(protocol.audio_reassembly_buf_.empty());
        protocol.HandleAudio(nullptr, 0, 0, 0, TAI_STREAM_ONE_SHOT, 1790000001000);
        assert(actual == input);
    }
}
'''
